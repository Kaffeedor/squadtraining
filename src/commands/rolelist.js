...
module.exports = {
    ...
    async execute(interaction, client) { // method for getting our interaction
        if (interaction.user.id != process.env.ADMIN_USERID_1 && interaction.user.id != process.env.ADMIN_USERID_2) return await interaction.reply({
            content: "This Command is only for bot-devs/-admins.",
            ephemeral: true
        });
        else {
            let i_reply = await interaction.reply({
            ...
            for (let i = 0; i < 5; i++) {
                ...
                for (let y = 0; y < temp_role_members.length; y++) {
                    n++; // add the member for each embed, three per line
                    if (n % 3 !== 0) {
